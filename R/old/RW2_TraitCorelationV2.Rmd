...
```{r}
df <- read_csv("../data/MB_Traits_processed_3.csv", show_col_types = FALSE)
#df <- read_csv("../data/MB_Traits_processed_4.csv", show_col_types = FALSE)
...
df2 <- select(df, price_in_usd, sum, trait_Shoes_Color, trait_Pants_Color, trait_Shirt_Color, trait_Overshirt_Color, trait_Hat_Color, trait_Glasses_Color, trait_Type, trait_Glasses, trait_Pants, trait_Hair_Style, trait_Shoes, trait_Shirt, trait_Hair_Color, trait_Overshirt, trait_Necklace, trait_Earring, trait_Beard, trait_Beard_Color, trait_Hat, trait_Jersey_Number)
...
```{r}
#df2 <- subset(df2, select = -c(trait_Type_Dissected))
```
...
corr_simple(data = df2, sig=0.1)
```

```{r}
reg = lm(price_in_usd ~ . -sum, data = df2)
#reg = lm(log(price_in_usd) ~ log(sum), data = df2)
summary(reg)
